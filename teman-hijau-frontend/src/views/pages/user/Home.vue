  ...
  <div class="grid">
    <div class="col-12 md:col-4">
      <div class="card mb-0 h-full">
        <div class="flex flex-column md:flex-row justify-content-between mb-3">
          <Avatar :image="$store.getters.getProfile?.profile_photo ? $store.getters.getProfile?.profile_photo : 'https://cdn-icons-png.flaticon.com/512/3940/3940403.png'" class="p-overlay-badge" size="xlarge" />
          <div class="text-right max-w-14rem">
            <span class="block text-900 text-xl font-medium mb-3">{{ $store.getters.getProfile?.first_name }} {{ $store.getters.getProfile?.last_name }}</span>
    ...
    <div class="col-12 md:col-6 lg:col-4">
      <div class="card mb-0 bg-bluegray-900">
        ...
        <span class="text-green-500 font-medium">{{ parseDate(wallet?.updated_at) }}</span>
      </div>
    </div>
      ...
      <div class="card mb-0 bg-purple-600">
        <div class="flex justify-content-between mb-3">
          <div>
            ...
            <div class="text-900 text-gray-100 font-medium text-xl">{{ $store.getters.getProfile?.pending_balance.balance }}</div>
          </div>
          <div class="flex align-items-center justify-content-center bg-white border-round"
            style="width: 2.5rem; height: 2.5rem">
        ...
        <span class="text-500 text-gray-300">Last updated at </span>
        <span class="text-green-500 font-medium">{{ parseDate($store.getters.getProfile?.pending_balance.updated_at) }}</span>
      </div>
    </div>
    ...
    <div class="col-12">
      <div class="card h-full">
        ...
        <DataTable v-model:filters="transactionFilter" :globalFilterFields="['garbage.name', 'qty', 'total_price', 'status']" dataKey="id" :value="$store.getters.getProfile.user_transactions" :rows="8" :paginator="true" responsiveLayout="scroll">
